...
class World:
    def __init__(self, numberOfRows, numberOfColumns):
        ...
        self.grid = [[0 for i in range(numberOfRows)] for j in range(numberOfColumns)]
        
    def update(self):
        newGrid = [[0 for i in range(self.numberOfRows)] for j in range(self.numberOfColumns)]
    ...
    def __hasGrainBelowRight(self, row, col):
        if col < self.numberOfColumns - 1 and col >= 0:
            return self.grid[col+1][row+1] == 1
        else:
            return True
    ...
    def addGrain(self, xposition, yposition, numberOfGrains):
